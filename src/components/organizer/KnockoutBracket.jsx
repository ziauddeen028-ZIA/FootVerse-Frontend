  ...
  return (
    <div
      className={`flex items-center justify-between px-3 py-2 transition-all ${
        isWinner
          ? 'bg-emerald-500/15 dark:bg-emerald-950/40 font-bold text-slate-900 dark:text-white'
          : isLoser
          ? 'opacity-40 grayscale-[20%] text-slate-500 dark:text-slate-400'
          : isTBD
        ...
        <div
          className={`w-6 h-6 rounded-md border flex items-center justify-center font-bold text-[10px] shrink-0 overflow-hidden ${
            isTBD
              ...
              : isWinner
              ? 'border-emerald-500/70 bg-emerald-500/25 text-emerald-600 dark:text-emerald-300 ring-1 ring-emerald-500/40'
              : badgeColors
          }`}
        >
          {team?.logoUrl ? (
        ...
        <span
          className={`w-6 h-6 rounded flex items-center justify-center text-xs tabular-nums font-black ${
            isWinner
              ...
              ? 'bg-slate-100 dark:bg-[#16261C] border border-slate-200/80 dark:border-[#1E3A29] text-slate-800 dark:text-slate-200'
  ...
  return (
    <div
      ...
      className={`group relative w-[220px] sm:w-[250px] rounded-xl transition-all duration-200 border bg-white dark:bg-[#101C14] ${
        ...
        isFinal
          ? 'border-emerald-500/60 dark:border-emerald-500/50 shadow-xl shadow-emerald-950/20 ring-1 ring-emerald-500/30'
          : isLive
    ...
    >
      ...
      <div
        className={`px-3 py-1.5 border-b flex items-center justify-between text-[10px] font-bold tracking-wider rounded-t-xl ${
          isFinal
            ? 'border-emerald-500/30 dark:border-[#1E3A29] bg-green-50/90 dark:bg-[#0B150E] text-green-700 dark:text-green-400'
            : 'border-slate-100 dark:border-[#1E3A29] text-slate-500 dark:text-slate-400 bg-transparent'
        }`}
      >
        <span className={`uppercase truncate max-w-[130px] flex items-center gap-1.5 ${isFinal ? 'font-black tracking-widest text-green-700 dark:text-green-400' : ''}`}>
          {isFinal && <Trophy className="w-3 h-3 text-green-600 dark:text-green-400 shrink-0" />}
          <span>{match.roundName || (isFinal ? 'Final' : `Match #${match.bracketPosition || 1}`)}</span>
        </span>
        ...
        {isLive ? (
          ...
          <span className="text-emerald-600 dark:text-emerald-400 font-bold uppercase">FT</span>
        ) : hasBothTeams ? (
      ...
      {/* Footer / Quick Action */}
      <div
        className={`px-3 py-1.5 border-t flex items-center justify-between text-[10px] rounded-b-xl ${
          isFinal
            ? 'border-emerald-500/20 dark:border-[#1E3A29] bg-slate-50/80 dark:bg-[#07130C]/80 text-slate-500 dark:text-slate-400'
            : 'border-slate-100 dark:border-[#1E3A29] text-slate-500 dark:text-slate-400 bg-slate-50/70 dark:bg-[#07130C]/50'
        }`}
      >
              ...
              <div key={`col-${round.name}-${rIndex}`} className="flex items-stretch">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <div className="mb-6 text-center">
                    <span
                      className={`text-xs font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-xs ${
                        isFinalRound
                          ? 'text-green-700 dark:text-green-400 bg-green-50 dark:bg-[#101C14] border border-green-500/40 dark:border-[#1E3A29] ring-1 ring-green-500/20'
                          : 'text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-[#101C14] border border-slate-200 dark:border-[#1E3A29]'
                      }`}
                    >
                      {round.name}
                    </span>
