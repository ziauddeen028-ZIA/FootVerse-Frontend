...
const PublicPlayerPanel = ({ playerId, onBack }) => {
  ...
  if (loading) return (
    <div className="saas-card rounded-2xl p-12 text-center text-slate-400 dark:text-slate-500 bg-white dark:bg-[#101C14] border border-slate-200 dark:border-[#1E3A29]">
      <div className="w-8 h-8 border-[3px] border-green-600 border-t-transparent rounded-full animate-spin mx-auto mb-3" />
      <p className="text-sm font-medium">Loading player statistics...</p>
    </div>
  );

  if (error) return (
    <div className="saas-card rounded-2xl p-8 text-center bg-white dark:bg-[#101C14] border border-red-200 dark:border-red-900/50">
      <p className="text-sm text-red-500 font-medium">{error}</p>
      <button onClick={onBack} className="mt-4 text-xs text-green-600 hover:text-green-700 font-semibold">← Back to search</button>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      {/* Back button */}
      <button onClick={onBack} className="flex items-center space-x-2 text-xs font-semibold text-slate-500 hover:text-green-600 dark:hover:text-green-400 transition-colors group">
      ...
      <div className="saas-card rounded-3xl p-6 sm:p-8 bg-gradient-to-r from-slate-900 via-[#0C1B12] to-[#07130C] text-white border border-slate-800 dark:border-[#1E3A29] shadow-xl relative overflow-hidden">
        <div className="absolute -top-10 -right-10 w-48 h-48 rounded-full bg-green-600/10 blur-3xl pointer-events-none" />
        <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6 relative z-10">
          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gradient-to-tr from-green-600 to-emerald-500 flex items-center justify-center text-white text-3xl sm:text-4xl font-black shadow-lg shadow-green-500/25 flex-shrink-0">
            {player?.fullName?.charAt(0)?.toUpperCase() || "P"}
          </div>
          <div className="text-center sm:text-left flex-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1.5">
              <span className="px-2.5 py-0.5 rounded-full bg-green-500/20 text-green-400 border border-green-500/30 text-xs font-bold">{player?.preferredPosition || "Player"}</span>
              {player?.jerseyNumber && (
                <span className="px-2.5 py-0.5 rounded-full bg-slate-800 text-slate-300 text-xs font-bold">Jersey #{player.jerseyNumber}</span>
              )}
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold font-heading text-white tracking-tight">{player?.fullName || "Athlete"}</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="saas-card rounded-2xl p-6 bg-white dark:bg-[#101C14] border border-slate-200/80 dark:border-[#1E3A29] shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Career Goals</span>
            <div className="text-3xl sm:text-4xl font-extrabold text-green-600 dark:text-green-400 mt-1">{data?.overallStats?.goals ?? 0}</div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-green-50 dark:bg-green-950/60 text-green-600 dark:text-green-400 flex items-center justify-center text-xl">⚽</div>
        </div>
        <div className="saas-card rounded-2xl p-6 bg-white dark:bg-[#101C14] border border-slate-200/80 dark:border-[#1E3A29] shadow-sm flex items-center justify-between">
          <div>
        ...
        <div className="saas-card rounded-2xl p-6 bg-white dark:bg-[#101C14] border border-slate-200/80 dark:border-[#1E3A29] shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-3xl sm:text-4xl font-extrabold text-teal-600 dark:text-teal-400 mt-1">{data?.overallStats?.tournamentsPlayed ?? 0}</div>
          </div>
      ...
      {data?.tournaments && data.tournaments.length > 0 && (
        <div>
          <div className="flex items-center space-x-2 mb-4">
            ...
            <h3 className="text-lg font-bold font-heading text-slate-900 dark:text-white">Tournament History</h3>
          </div>
                ...
                <div key={entry.tournament.id} onClick={() => handleSelectTournament(entry.tournament.id)}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-950 px-2 py-0.5 rounded-full">{entry.tournament.format || "Knockout"}</span>
                    ...
                    {isSelected && <div className="w-6 h-6 rounded-full bg-green-600 text-white flex items-center justify-center"><CheckCircle2 className="w-4 h-4" /></div>}
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100 dark:border-[#1E3A29] flex items-center justify-between text-xs">
                    ...
                    <span className="text-slate-500">Goals: <strong className="text-green-600 dark:text-green-400">{entry.tournamentStats.goals}</strong></span>
                    <span className="text-green-600 dark:text-green-400 font-semibold flex items-center">Details <ChevronRight className="w-3 h-3 ml-0.5" /></span>
                  </div>
                </div>
              );
            })}
        ...
        <div ref={detailsRef} id="detailed-tournament-performance" className="saas-card rounded-3xl p-6 sm:p-8 bg-white dark:bg-[#101C14] border border-green-500/30 dark:border-[#1E3A29] shadow-lg space-y-6 scroll-mt-24">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-[#1E3A29] pb-5">
            <div>
              ...
              <h3 className="text-xl sm:text-2xl font-black font-heading text-slate-900 dark:text-white mt-0.5">{activeTournament.tournament.name}</h3>
            ...
            <div className="flex items-center space-x-3">
              <div className="px-4 py-2 rounded-xl bg-green-50 dark:bg-green-950/60 border border-green-100 dark:border-green-900 text-center">
                <span className="text-[10px] font-bold uppercase text-slate-400">Goals</span>
                <div className="text-xl font-black text-green-600 dark:text-green-400">{activeTournament.tournamentStats.goals}</div>
              </div>
              <div className="px-4 py-2 rounded-xl bg-slate-50 dark:bg-[#16261C] border border-slate-200 dark:border-[#1E3A29] text-center">
          ...
          <div className="flex items-center space-x-2 px-3 py-2 rounded-xl bg-slate-50 dark:bg-[#16261C] border border-slate-200 dark:border-[#1E3A29] text-xs text-slate-500">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {activeTournament.matches.map((m) => (
                  <div key={m.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-[#16261C] border border-slate-200/70 dark:border-[#1E3A29] gap-3">
                    <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center justify-between sm:justify-end space-x-4">
                      ...
                      {m.playerGoals > 0 ? (
                        <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg bg-green-500/10 text-green-700 dark:text-green-400 text-xs font-bold border border-green-500/20">
...
export const PlayerSearchView = ({ initialPlayerId = null }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Search bar */}
      <div className="saas-card rounded-2xl p-4 sm:p-5 bg-white dark:bg-[#101C14] border border-slate-200 dark:border-[#1E3A29] shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-[#16261C] border border-slate-200 dark:border-[#1E3A29] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-600 text-slate-900 dark:text-white"
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {players.map((p) => (
            <button
              ...
              className="saas-card rounded-2xl p-5 bg-white dark:bg-[#101C14] border border-slate-200/80 dark:border-[#1E3A29] hover:border-green-500 hover:shadow-md transition-all text-left group"
            >
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-green-600 to-emerald-500 flex items-center justify-center text-white text-lg font-black flex-shrink-0">
                  {p.fullName?.charAt(0)?.toUpperCase() || "P"}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white truncate group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">
                    {p.fullName}
                  </h3>
              ...
              <div className="mt-4 flex items-center justify-between text-xs">
                <div className="flex items-center space-x-3">
                  {p.goals > 0 && (
                    <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-full bg-green-500/10 text-green-700 dark:text-green-400 font-bold border border-green-500/20">
